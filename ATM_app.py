...
if theme == "Dark Mode":
    st.markdown("""
        ...
        h1, h2, h3 {color: #e2e8f0 !important; font-family: 'Poppins', sans-serif;}
        p, span, div, li, text {color: #cbd5e1 !important;}
    ...
    st.markdown("""
        <style>
        .main {background-color: #F8FAF1;}
        h1, h2, h3 {color: #001F3F !important; font-family: 'Poppins', sans-serif;}
        p, span, div, li {color: #1E293B;}
        .insight-box {background-color: #ffffff; color: #1E293B; border-left: 5px solid #11CAA0; padding: 20px; border-radius: 5px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); margin-bottom: 15px;}
        </style>
        """, unsafe_allow_html=True)
...
with tab1:
    ...
    if filtered_df.empty:
        ...
        with col1:
            ...
            fig_hol.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)")
...
with tab2:
    ...
    if filtered_df.empty or len(filtered_df) < 3:
        st.warning("⚠️ Not enough data points available for clustering. Try expanding your filters.")
    else:
        ...
        with col_c1:
            ...
            fig_cluster.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", margin=dict(l=0, r=0, b=0, t=40))
...
with tab3:
    ...
    if filtered_df.empty or len(filtered_df) < 5:
         st.warning("⚠️ Not enough data points available for Anomaly Detection. Please adjust your sidebar filters.")
    else:
